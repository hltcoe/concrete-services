...
service ResultsServer extends services.Service {
   ...
   */
  void registerSearchResult(1: search.SearchResults results, 2: services.AnnotationTaskType taskType) throws (1: services.ServicesException ex)
   ...
   */
  search.SearchResults getLatestSearchResult(1: string userId) throws (1: services.ServicesException ex)

  /**
   * Get a search results object
   */
  search.SearchResults getSearchResult(1: uuid.UUID searchResultsId) throws (1: services.ServicesException ex)

  /**
   * Start an annotation session
   * Returns a session id used in future session calls
   */
  uuid.UUID startSession(1: uuid.UUID searchResultsId) throws (1: services.ServicesException ex)
